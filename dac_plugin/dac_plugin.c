...
static int hifi_dac_set_output_devices(struct amplifier_device* device, void* adev, uint32_t devices) {
    ...
    struct audio_device *current_adev = (struct audio_device *)adev;
    ...
    if(want_to_enable_hifi_dac) {
        platform_set_snd_device_backend(SND_DEVICE_OUT_HEADPHONES, HIFI_DAC_BACKEND, 
            HIFI_DAC_INTERFACE);
        audio_route_apply_and_update_path(current_adev->audio_route, HIFI_DAC_MIXER_PATH);
    ...
    // In all other cases, go back to normal state
        ...
        platform_set_snd_device_backend(SND_DEVICE_OUT_HEADPHONES, DEFAULT_BACKEND, 
            DEFAULT_INTERFACE);
        audio_route_apply_and_update_path(current_adev->audio_route, 
            DEFAULT_MIXER_PATH);
...
static int hifi_dac_set_parameters(struct amplifier_device *device, void *adev,
    ...
    if (err >= 0) {
        ...
        list_for_each(node, &current_adev->usecase_list) {
            ...
            if (usecase->stream.out && (usecase->type == PCM_PLAYBACK) &&
                ...
                */
                ...
                select_devices(current_adev, usecase->id);
