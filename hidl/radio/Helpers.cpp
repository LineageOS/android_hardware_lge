...
V1_4::SignalStrength Create1_4SignalStrength(const V1_0::SignalStrength& sigStrength) {
    ...
    newSigStrength.tdscdma.rscp =
            sigStrength.tdScdma.rscp != INT_MAX ? -sigStrength.tdScdma.rscp + 120 : INT_MAX;
...
hidl_vec<V1_4::CellInfo> Create1_4CellInfoList(const hidl_vec<V1_0::CellInfo>& cellInfo) {
    ...
    for (int x = 0; x < cellInfo.size(); ++x) {
        ...
        if (cellInfo[x].gsm.size() == 1) {
            ...
            TdscdmaInfo.signalStrengthTdscdma.rscp =
                    cellInfo[x].tdscdma[0].signalStrengthTdscdma.rscp != INT_MAX
                            ? -cellInfo[x].tdscdma[0].signalStrengthTdscdma.rscp + 120
                            : INT_MAX;
...
hidl_vec<hidl_string> DelimitedStrToVec(std::string delimitedStr) {
    ...
    for (; iter != end; ++iter) tokens.push_back(hidl_string(*iter));

    return hidl_vec(tokens);
}

V1_4::SetupDataCallResult Create1_4SetupDataCallResult(
